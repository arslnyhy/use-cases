        ...
        if match:
            ...
            if existing_vlans:
                if vlan_name not in existing_vlans:
                    update_record("cisco_ios", "vlan", existing_vlans +
                                  f"\n{vlan_number} {vlan_name} {vlan_status} {vlan_ports}")
            else:
                create_record("cisco_ios", "vlan", f"{vlan_number} {
                              vlan_name} {vlan_status} {vlan_ports}")
            return True
    ...
    if command == "show vlan brief":
        ...
        if vlans:
            ...
            for vlan in vlans.split("\n"):
                ...
                vlan_row = "{:<4} {:<32} {:<9} {:<30}".format(
                    vlan_details[0], vlan_details[1], vlan_details[2], vlan_details[3])
